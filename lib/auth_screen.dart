...
//                         bool isSelected = _selectedSensorIds.contains(
//                           sensor.id,
//                         );
...
//         builder: (context) => MaintenanceScreen(activeVehicle: _activeVehicle!)
//       ));
//     },
//   ),
          
//           if (!_isConnectedToCar && !_showWifiConfig)
//             IconButton(
...
//                     ScaffoldMessenger.of(context).showSnackBar(
//                       const SnackBar(
//                         content: Text('Connecting to Bluetooth...'),
//                       ),
//                     );
//                     _setupRealtimeChannel();
//                     bool success = await _obdService.connectToBluetooth(
//                       _devices[index],
//                     );
//                     if (mounted) {
...
//           const SizedBox(height: 20),
//           const Text(
//             "Ensure your phone is connected to the OBD-II adapter's Wi-Fi network.",
//             textAlign: TextAlign.center,
//             style: TextStyle(color: Colors.white70),
//           ),
//           const SizedBox(height: 30),
...
//               ScaffoldMessenger.of(context).showSnackBar(
//                 const SnackBar(
//                   content: Text('Establishing TCP Socket Connection...'),
//                 ),
//               );
//               _setupRealtimeChannel();
//               bool success = await _obdService.connectToWiFi(
...
//                   ScaffoldMessenger.of(context).showSnackBar(
//                     const SnackBar(
//                       content: Text('TCP Connection failed.'),
//                       backgroundColor: Colors.red,
//                     ),
//                   );
//                 }
...
//                               activeVehicle:
//                                   _activeVehicle, // <--- JUST ADD THIS LINE!
//                             ),
//                           ),
//                         );
//                       },
...
import 'dtc_screen.dart'; 
import 'safety_screen.dart'; 
import 'maintenance_screen.dart'; // <--- The new Maintenance Log
...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  bool _isPollingData = false; 
  Timer? _dbLogTimer; 
  bool _hasTriggeredMaintenanceAlert = false; // <--- Prevents notification spam
    ...
    _obdService.obdDataStream.listen((data) {
      if (mounted) {
        setState(() {
          ...
          // ---> CHECK MAINTENANCE STATUS ON NEW DISTANCE DATA <---
          if (data['type'] == 'DISTANCE') _checkMaintenanceStatus(data['value']);
        });
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        ...
        action: SnackBarAction(label: 'VIEW', textColor: Colors.white, onPressed: () {
          Navigator.of(context).push(MaterialPageRoute(builder: (context) => MaintenanceScreen(activeVehicle: _activeVehicle!)));
        }),
      )
    );
  }
  ...
  void _startPollingData() async {
    ...
    while (_isConnectedToCar && _isPollingData) {
      ...
      await _obdService.sendCommand('0131\r'); await Future.delayed(const Duration(milliseconds: 150)); // Fetch Distance
    }
  }
          ...
          IconButton(
            ...
            onPressed: () async {
              ...
              if (selectedCar != null && mounted) {
                setState(() {
                  _activeVehicle = selectedCar as Map<String, dynamic>;
                  _hasTriggeredMaintenanceAlert = false; // Reset the alert trigger when a new car is picked
                });
